...
video_folder = os.path.join(script_dir, f"videos/{name}")
...
with torch.no_grad():
    for i in range(0, len(img1_raw), batch_size):
        ...
        #moving to gpu (figure out torch.multiproces later)
...
# save (NOTE: this seems to consume a lot of space. look into precision redution methods..)
with open(f"optical_flow_tensors/{name}_flow.pkl", "wb") as f:
    pickle.dump(predicted_flows.cpu().numpy(), f)

print(f"optical flow for {name} saved. shape is: {predicted_flows.shape}")



'''
uncomment BELOW to get video comparisons
'''
