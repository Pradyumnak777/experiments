...
from utils.custom_vid import Video
...
def main(args):
    ...
    '''
    gt-tracjectory, visible WILL NOT BE AVAILABLE!!
    '''
    ...
    for j, (video, query_points_i, video_ori) in enumerate(dataloader): #CHANGE MADE!
        ...
        '''
        THIS wont be possible too!! we dont have ground truth!
        '''
        # if args.vis_video:
        #     _, _, _, H_ori, W_ori = video_ori.shape
        ...
        while len(visited_idx) < T - 1:
            ...
            #     print("skipping..")
            #     count += 1
            #     continue #cuz we already have this (temporary)
        ...
        # Convert query points [t, y, x] (pixel) -> latent pixel coords
        queried_coords_latent = query_points_i.clone().float()
        queried_coords_latent[:, :, 1] /= latent_scaling_size  # y
        queried_coords_latent[:, :, 2] /= latent_scaling_size  # x
        ...
        '''
        THIS BELOW TRAJ MIGHT BE USEFUL FOR PLOTTING A GRAPH..PERIODIC MOTION!!
        '''
        ...
        '''
        THIS will be needed as this is the answer/prediction
        '''
        if args.vis_video:
            vis_traj = trajectory.clone()
            vis_traj[..., 0] *= (W_ori / W)
            vis_traj[..., 1] *= (H_ori / H)
            # 'LINEWIDTH' below controls point thicknes...reduce it!
...
if __name__ == "__main__":
    ...
    sys.argv = [
        ...
        "--inverse_step", "49",
        "--output_dir", "./custom_vid_output",
        "--vid_root", "videos", 
        "--resize_h", "480",
        "--resize_w", "720",
        "--chunk_frame_interval",         
        "--average_overlapped_corr",
        "--pipe_device", "cuda:0",
        "--vis_video",
        "--tracks_leave_trace", "1" #FOR TRAIL!!
    ]
    ...
    parser.add_argument('--vid_root', type=str)
